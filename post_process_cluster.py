...
def main():
    ...
    input_formatted = input.pivot_table(
        index=["ds", "cluster_subsampling_ratio", "p", "subsampling_ratio"],
        columns="clf",
        values=perf,
        aggfunc="first",
    )
    ...
    if args.wilcoxon:
        ...
        qs = np.sort(
            input_formatted.index.unique(level="cluster_subsampling_ratio").values
        )
        for p in ps:
    ...
    # No noise
    no_noise = input[
        (input["cluster_subsampling_ratio"] == 1) & (input["corruption"] == "uniform")
    ]
    no_noise = no_noise.drop(["cluster_subsampling_ratio", "corruption"], axis=1)
    ...
    no_covariate_table = round(
        no_covariate_formatted.groupby(["ds", "p"])
        .agg(["mean", "std"])
        .groupby(["p"])
        .mean()
        * 100,
    ...
    output_directory_no_covariate = os.path.join(args.output_directory, "cluster")
    if not os.path.exists(output_directory_no_covariate):
        os.mkdir(output_directory_no_covariate)

    avg_perfs = no_covariate_formatted.groupby("ds").mean()

    if args.corr:
        stats_names = stats.columns
        corrs = avg_perfs.join(stats).corr()[stats_names].drop(stats_names, axis=0)

        with open(os.path.join(output_directory_no_covariate, "corrs.tex"), "w") as tf:
            tf.write(corrs.to_latex())

        with open(os.path.join(output_directory_no_covariate, "stats.tex"), "w") as tf:
    ...
    if args.nemenyi:
        ...
        plt.savefig(
            os.path.join(output_directory_no_covariate, "nemenyi.pdf"),
            bbox_inches="tight",
            format="pdf",
        )

        cluster_subsampling_ratios = np.sort(
            input_formatted.index.unique(level="cluster_subsampling_ratio").values
        )

        for cluster_subsampling_ratio in cluster_subsampling_ratios:
            avg_perfs = (
                no_covariate_formatted.loc[
                    (
                        slice(None),
                        slice(None),
                        cluster_subsampling_ratio,
                        slice(None),
                    ),
                    :,
                ]
            ...
            plt.savefig(
                os.path.join(
                    output_directory_no_covariate,
                    f"nemenyi-{cluster_subsampling_ratio}.pdf",
                ),
                bbox_inches="tight",
                format="pdf",
            )
    ...
    if args.wilcoxon:
        ps = np.sort(no_covariate_formatted.index.unique(level="p").values)
        qs = np.sort(
            no_covariate_formatted.index.unique(
                level="cluster_subsampling_ratio"
            ).values
        )
        for competitor1, competitor2 in itertools.product(competitors, competitors):
            if competitor1 == competitor2:
            ...
            for p in ps:
                zs = []
                for q in qs:
                    score1 = no_covariate_formatted.loc[
                        (slice(None), p, q), competitor1
                    ].values
                    score2 = no_covariate_formatted.loc[
